...
function buildSnapPayload(order, baseUrl) {
  ...
  const customerAddress = order.name_tag || order.customer_address || order.address || '-';
  ...
  // Callback/redirect URL yang aman: utamakan parameter baseUrl dari request,
  // lalu env BASE_URL/APP_URL; terakhir kosong (bukan "undefined").
  const appUrl = (baseUrl || process.env.BASE_URL || process.env.APP_URL || '').replace(/\/+$/, '');
  ...
  return {
    ...
    callbacks: {
      finish: `${appUrl}/order-success`,
      error: `${appUrl}/checkout?error=payment_failed`,
      pending: `${appUrl}/order-success?status=pending`
    }
  };
}
