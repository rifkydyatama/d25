...
class Midtrans {
  ...
  async getStatus(orderId) {
    return await this.core.transactions.status(orderId);
  }

  // Handle notification (callback)
  async handleNotification(notificationJson) {
    return await this.core.transactions.notification(notificationJson);
  }

  // Approve/deny challenge (3DS)
  async approveChallenge(orderId, payload) {
    return await this.core.transactions.approve(orderId, payload);
  }

  // Cancel transaction
  async cancel(orderId) {
    return await this.core.transactions.cancel(orderId);
  }

  // Expire transaction
  async expire(orderId) {
    return await this.core.transactions.expire(orderId);
  }

  // Refund transaction
  async refund(orderId, payload) {
    return await this.core.transactions.refund(orderId, payload);
  }
}
...
function buildSnapPayload(order) {
  ...
  const dpPercent = order.dp_percentage || order.dpPercentage || 50;
  const dpAmount = Number(order.dp_amount || order.dpAmount || (isPreorder ? Math.round(subtotal * (dpPercent / 100)) : 0));
