...
const productService = {
  ...
  async update(id, updates) {
    ...
    if (updates.imageUrl || updates.image) updateData.image_url = updates.imageUrl || updates.image;
...
const cartService = {
  ...
  async getOrCreateCart(userId, sessionId) {
    ...
    if (userId) {
      const { data: existingCart } = await getDb()
        .from('carts')
        .select('*')
        .eq('user_id', userId)
        .single();
      ...
      if (existingCart) {
        ...
        const { data: newCart, error } = await getDb()
          .from('carts')
  ...
  async mergeCarts(userId, sessionId) {
    const { data: guestCart } = await getDb()
      .from('carts')
      .select('id')
      .eq('session_id', sessionId)
      .is('user_id', null)
...
const orderService = {
  ...
  async createFromCart(cartId, orderData) {
    ...
    const dpAmount = isPreorder ? Math.round(subtotal * dpPercentage / 100) : 0;
